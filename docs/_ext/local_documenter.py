...
class LocalFunctionDocumenter(FunctionDocumenter):
    def format_name(self) -> str:
        ...
        if hasattr(self.object, "module_name_override"):
            self.objpath = self.object.module_name_override.split(".") + [
                self.objpath[-1]
            ]
...
def setup(app):
    setup.app = app
    app.add_autodocumenter(LocalFunctionDocumenter, override=True)
    app.add_autodocumenter(LocalModuleDocumenter, override=True)
