...
def save_plotly_figure(fig, path):
    ...
    fig_html = plotly.offline.plot(
        fig, output_type="div", include_plotlyjs="cdn", auto_open=False
    )
...
def create_directive_block(name, arguments, options, content):
    ...
    if len(options.items()) > 0:
        ...
        options_block = "\n".join(
            ":{}: {}".format(k, process_value(v)) for k, v in options.items()
        )
...
def run_code(code, code_path, ns=None, function_name=None, fig_vars=None):
    ...
    try:
        ...
        if ends_with_show(code):
            ...
            figs = (
                [ns[fig_var] for fig_var in fig_vars]
                if fig_vars
                else [ns[variable_name]]
            )
        elif function_name is not None:
            exec(code, ns)
            exec(
                assign_last_line_into_variable(function_name + "()", variable_name), ns
            )
...
def run(arguments, content, options, state_machine, state, lineno):
    ...
    for j, (code_piece, figures) in enumerate(results):
        ...
        if config.plotly_include_directive_source:
            directive_source = create_directive_block(
                "plotly", arguments, options_copy, content
            )
